...
export default function UnreadMessages({ onSelectChat }) {
  ...
  const conversations = Array.from({ length: 20 }, (_, i) => ({
    ...
    messages: [
      { id: 1, sender: "User", message: "I'd like to know if I can access your services here. Is TUMA available in Uganda?", timestamp: formatTime(new Date(Date.now() - 300000)) },
      { id: 2, sender: "TUMA Support", message: "Hello! Yes, TUMA is available in Uganda. How can we assist you today?", timestamp: formatTime(new Date(Date.now() - 280000)) },
      { id: 3, sender: "User", message: "That’s great! I’m interested in using TUMA for sending money. What’s the process?", timestamp: formatTime(new Date(Date.now() - 250000)) },
      { id: 4, sender: "TUMA Support", message: "It’s simple! You can download our app or visit our website, create an account, and start sending money instantly. We offer competitive rates and secure transactions.", timestamp: formatTime(new Date(Date.now() - 230000)) },
    ],
  }));
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      setNeedsLoadMore(scrollHeight > containerHeight || visibleCount < conversations.length);
    }
  }, [visibleCount]);
